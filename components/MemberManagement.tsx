    ...
    <div className="flex-1 p-6 bg-slate-100 overflow-y-auto">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">센터의 모든 회원을 관리합니다.</p>
        </div>
        {canManageMembers && (
            <button onClick={onAddMember} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-md shadow-sm text-sm font-medium hover:bg-blue-700">
                <PlusIcon className="w-4 h-4" />
                신규 회원 추가
            </button>
        )}
      </div>
      <div className="mb-4 flex items-center justify-between gap-4 p-4 bg-white rounded-lg shadow-sm">
        <button onClick={handleDownloadCSV} className="flex items-center gap-2 px-3 py-2 bg-green-600 text-white rounded-md shadow-sm text-sm font-medium hover:bg-green-700">
